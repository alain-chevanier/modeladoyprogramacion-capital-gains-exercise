...
Nota: Los números de punto flotante (~float~, ~double~) pueden introducir errores de redondeo en algunos casos. Si decides usar estos tipos, asegúrate de que tu solución pase todos los tests. Otra opción es usar tipos que manejen decimales con precisión, como ~BigDecimal~ en Java.

Para que un ~BigDecimal~ mantenga los ceros a la derecha, puedes usar el método ~new BigDecimal(number).setScale(2, RoundingMode.HALF_UP);~, donde ~number~ es una cadena que contiene un número decimal, por ejemplo ~"1000.00"~.
...
* Leer y escribir JSON
Para leer y escribir ~JSON~ utiliza la biblioteca ~Jackson~, dicha biblioteca ya se encuentra incluida en la base de código que recibes para resolver este problema.

Para escribir un objeto a ~JSON~:
#+begin_src java
...
List<Tax> listOfTaxes = List.of(new Tax("0.00"), new Tax("10000.00"));
ObjectMapper objectMapper = new ObjectMapper();
String jsonString = objectMapper.writeValueAsString(listOfTaxes);
System.out.println(jsonString);
// Output: [{"tax":0.00},{"tax":10000.00}]
#+end_src

Para leer un objeto desde ~JSON~:
#+begin_src java
...
ObjectMapper objectMapper = new ObjectMapper();
List<Operation> operations = objectMapper.readValue(jsonInput, new TypeReference<List<Operation>>() {});
#+end_src
